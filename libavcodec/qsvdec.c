...
static int qsv_decode_init_context(AVCodecContext *avctx, QSVContext *q, mfxVideoParam *param)
{
    ...
    if (ret < 0) {
        if (param->mfx.FrameInfo.Width != 0 && param->mfx.FrameInfo.Height != 0)
            return ff_qsv_print_error(avctx, ret, "Error initializing the MFX video decoder");
        return ret;
    }
...
static int parse_sei_hevc(AVCodecContext* avctx, QSVContext* q, QSVFrame* out)
{
    ...
    while (1) {
        ...
        if (init_get_bits(&gb, &payload.Data[start], payload.NumBit - start * 8) < 0)
            ...
            ret = ff_hevc_decode_nal_sei(&gb, avctx, &q->sei, &ps, HEVC_NAL_SEI_PREFIX);
    ...
    if (out && out->frame)
        return ff_set_side_data(avctx, &q->sei, NULL, out->frame);
