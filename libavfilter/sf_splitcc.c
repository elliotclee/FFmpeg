...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (sd && (s->had_keyframe || frame->key_frame)) {
        ...
        if (got_output) {
            int64_t next_scatter_pts = s->next_sub_frame ? s->next_sub_frame->pts : 0;
            ...
            if (s->real_time && s->scatter_realtime_output) {
                if (next_scatter_pts)
                    s->next_sub_frame->pts = next_scatter_pts;
    ...
    if (s->real_time && s->scatter_realtime_output && s->next_sub_frame && s->next_sub_frame->pts > 0 && frame->pts >= s->next_sub_frame->pts) {
        s->new_frame = 1;
