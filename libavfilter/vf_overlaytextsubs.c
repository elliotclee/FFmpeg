...
static void process_header(AVFilterContext *ctx, AVFrame *frame)
{
    TextSubsContext *s = ctx->priv;
    ...
    if (frame && frame->subtitle_header) {
        ...
        if (!s->original_w && !s->original_h) {
            s->original_w = s->track->PlayResX;
            s->original_h = s->track->PlayResY;
        }
    }
    else {
    ...
    if (s->original_w && s->original_h) {
        ass_set_pixel_aspect(s->renderer, (double)inlink->w / inlink->h /
                             ((double)s->original_w / s->original_h));
        ass_set_storage_size(s->renderer, s->original_w, s->original_h);
    } else
        ass_set_storage_size(s->renderer, inlink->w, inlink->h);
...
static int textsub2video_request_frame(AVFilterLink *outlink)
{
    ...
    AVFrame *out;
    ASS_Image *image;
    uint8_t rgba_tranparent[] = { 0, 0, 0, 0 };
    FFDrawColor color;
    ...
    if (s->render_latest_only && outlink->current_pts != AV_NOPTS_VALUE && next_pts < s->current_input_pts)
        next_pts = s->current_input_pts;
    ...
    ff_draw_color(&s->draw, &color, rgba_tranparent);
    ff_fill_rectangle(&s->draw, &color, out->data, out->linesize, 0, 0, out->width, out->height);
...
static int textsub2video_filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    av_log(ctx, AV_LOG_VERBOSE, "textsub2video_filter_frame num_subtitle_rects: %d, frame-pts: %"PRId64", start_time_ms: %"PRId64"\n", frame->num_subtitle_areas, frame->pts, start_time);
    ...
    s->current_input_pts = frame->pts;

exit:
    av_frame_free(&frame);

    if (s->need_frame || s->render_latest_only) {
        s->need_frame = 0;
...
static const AVOption overlaytextsubs_options[] = {
    ...
    {"original_size",      "set the size of the original video (used to scale fonts)",
                                                                 OFFSET(original_w),         AV_OPT_TYPE_IMAGE_SIZE, {.str = NULL}, 0,         0,        .flags =  FLAGS},
    { .name = NULL }
};
