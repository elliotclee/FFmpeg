...
static int insert_trim(int64_t start_time, int64_t duration, int64_t end_time,
...
static int configure_output_subtitle_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    if (of->start_time != AV_NOPTS_VALUE && of->recording_time != INT64_MAX)
        end_time = of->start_time + of->recording_time;

    ret = insert_trim(of->start_time, INT64_MAX, end_time,
...
static int configure_output_video_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    snprintf(name, sizeof(name), "trim_out_%d_%d",
             ost->file_index, ost->index);
    ret = insert_trim(of->start_time, of->recording_time, AV_NOPTS_VALUE,
                      &last_filter, &pad_idx, name);
    if (ret < 0)
        return ret;
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (f->recording_time != INT64_MAX)
        end_time = tsoffset + f->recording_time;

    ret = insert_trim(((f->start_time == AV_NOPTS_VALUE) || !f->accurate_seek) ?
                      AV_NOPTS_VALUE : tsoffset, INT64_MAX, end_time,
                      &last_filter, &pad_idx, name);
    if (ret < 0)
        return ret;
                                        ...
                                        AVFilterInOut *in)
{
    ...
    ret = insert_trim(((f->start_time == AV_NOPTS_VALUE) || !f->accurate_seek) ?
                      AV_NOPTS_VALUE : tsoffset, f->recording_time, AV_NOPTS_VALUE,
                      &last_filter, &pad_idx, name);
    if (ret < 0)
        return ret;
