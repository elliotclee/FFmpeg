...
static void check_print_graphs(int nocheck)
{
    if (filtergraphs_printed)
        return;

    /* print filter graph details */
...
static void do_subtitle_out(OutputFile *of, OutputStream *ost, AVFrame *frame)
{
    ...
    if (frame->repeat_sub) {
        av_log(NULL, AV_LOG_DEBUG, "Ignoring repeated subtitle frame\n");
        return;
    }
...
static int alloc_extradata(AVCodecParameters *par, int size)
{
                         ...
                         AVFrame *next_picture)
{
    ...
    int64_t next_picture_pkt_pts = next_picture ? next_picture->pts : AV_NOPTS_VALUE;
    ...
    if (!next_picture) {
        ...
        // Emby Custom
        if (ifile != NULL && (ifile->skip_interval != AV_NOPTS_VALUE || ifile->nb_skip_times > 0)) {
            double precisionDelay;;
            int64_t next_pkt_ts = (next_picture_pkt_pts != AV_NOPTS_VALUE) ? next_picture_pkt_pts : next_picture->pkt_dts;
            double next_pkt_time = next_pkt_ts * av_q2d(ist->st->time_base) * AV_TIME_BASE;

            if (ifile->nb_skip_times > 0)
                precisionDelay = 0;
            else if (ifile->skip_interval >= 10 * AV_TIME_BASE)
                precisionDelay = 0.5 * AV_TIME_BASE;
            else if (ifile->skip_interval >= 5 * AV_TIME_BASE)
                precisionDelay = 0.4 * AV_TIME_BASE;
            else
                precisionDelay = 0.2 * AV_TIME_BASE;

            if (ifile->nb_skip_times > 0) {
                if (next_pkt_time < is->skip_position + precisionDelay)
                    nb_frames = 0;
            }
            else if (next_pkt_time < is->skip_last_output + precisionDelay) {
                nb_frames = 0;
                ////av_log(NULL, AV_LOG_INFO, "Skip-Option - Precision-Skip\n");
            }
            ...
            if (nb_frames > 0) {
                is->skip_last_output = next_pkt_time;
                av_log(NULL, AV_LOG_DEBUG, "Skip-Option - Select output: skip_pos:%s "
                    "input_pts_time:%s rebased:%s\n",
                    av_ts2timestr(is->skip_position, &ist->st->time_base),
                    av_ts2timestr(next_picture_pkt_pts, &ist->st->time_base),
                    av_ts2timestr(next_pkt_time - is->start_time, &AV_TIME_BASE_Q));
            }
            else
                av_log(NULL, AV_LOG_INFO, "Skip-Option - SKIP   output: skip_pos:%s "
                    "input_pts_time:%s rebased:%s\n",
                    av_ts2timestr(is->skip_position, &ist->st->time_base),
                    av_ts2timestr(next_picture_pkt_pts, &ist->st->time_base),
                    av_ts2timestr(next_pkt_time - is->start_time, &AV_TIME_BASE_Q));
    ...
    for (i = 0; i < nb_frames; i++) {
        ...
        // Emby Custom
        if (in_picture && is != NULL && ifile != NULL && ost->file_index == 0
            && (ifile->skip_interval != AV_NOPTS_VALUE || ifile->nb_skip_times > 0)) {
            ...
            if (ret < 0) {
                const char *type_desc = av_get_media_type_string(enc->codec_type);
                av_log(NULL, AV_LOG_ERROR, "Error submitting video frame (via skiplist) to the encoder\n", type_desc);
                exit_program(1);
            }
            ...
            while (1) {
                ret = avcodec_receive_packet(enc, pkt);
                update_benchmark("%s_%s %d.%d", "encode", "video",
                         ost->file_index, ost->index);

                if (ost->st->codecpar->extradata_size == 0 && ost->enc_ctx->extradata_size > 0) {

                    ret = alloc_extradata(ost->st->codecpar, ost->enc_ctx->extradata_size);
                    if (ret < 0) {
                        av_log(NULL, AV_LOG_WARNING, "Unable to add extradata to output parameters. Output segments may be invalid.\n");
                        exit_program(1);
                    }
                    memcpy(ost->st->codecpar->extradata, ost->enc_ctx->extradata, ost->enc_ctx->extradata_size);

                    av_log(NULL, AV_LOG_VERBOSE, "Setting codecpar->extradata from enc_context_extradata, size: %d\n", ost->st->codecpar->extradata_size);
                }
                ...
                // Emby Custom
                if (is != NULL && ifile != NULL && ost->file_index == 0
                    && (ifile->skip_interval != AV_NOPTS_VALUE || ifile->nb_skip_times > 0)) {

                    av_log(NULL, AV_LOG_INFO, "Skip-Option - Write output: "
                        "pkt_pts_time:%s pkt_dts_time:%s input_pts_time:%s\n",
                        av_ts2timestr(pkt->pts, &ost->mux_timebase),
                        av_ts2timestr(pkt->dts, &ost->mux_timebase),
                        av_ts2timestr(next_picture_pkt_pts, &ist->st->time_base));
                }
                ...
                // Emby Custom
                if (is != NULL && ifile != NULL && ifile->skip_interval != AV_NOPTS_VALUE && ost->file_index == 0) {

                    if (is->skip_position == 0) {
                        is->skip_position = is->start_time;
                    }

                    is->skip_position += ifile->skip_interval;

                    if (is->skip_position > is->skip_last_output + 0.3 * AV_TIME_BASE) {


                        for (i = 0; i < ifile->nb_streams; i++) {
                            AVCodecContext *avctx1;
                            InputStream* ist1 = input_streams[ifile->ist_index + i];
                            avctx1 = ist1->dec_ctx;
                            if (ist1->decoding_needed) {
                                avcodec_flush_buffers(avctx1);
                            }
                        }

                        ret = av_seek_frame(is, -1, is->skip_position, 0); // AVSEEK_FLAG_ANY
                        ////av_log(NULL, AV_LOG_INFO, "Seek to %d\n", is->skip_position);
                        if (ret < 0)
                            av_log(NULL, AV_LOG_DEBUG, "Seek error %d\n", ret);
                    }

                } else if (is != NULL && ifile != NULL && ost->file_index == 0 && ifile->nb_skip_times > 0) {
                    int64_t seek_timestamp;

                    if (is->skip_index < ifile->nb_skip_times) {
                        is->skip_position = ifile->skip_times[is->skip_index];
                        is->skip_index++;
                    }
                    else {
                        is->skip_position = INT64_MAX;
                        ost->max_frames = 1;
                    }
                    ...
                    if (ist->nb_filters > 0) {
                        FilterGraph *fg = ist->filters[0]->graph;
                        AVFilterGraph *afg = fg->graph;
                        AVFilterContext* fctx = NULL;

                        for (i = 0; i < afg->nb_filters; i++)
                            if (afg->filters[i]->filter->name && !strcmp("trim", afg->filters[i]->filter->name))
                                fctx = afg->filters[i];

                        if (fctx) {
                            AVFilterLink *inlink = fctx->inputs[0];
                            int64_t filter_start_pts = av_rescale_q(is->skip_position, AV_TIME_BASE_Q, inlink->time_base);

                            ret = av_opt_set_int(fctx, "start_pts", filter_start_pts, AV_OPT_SEARCH_CHILDREN);
                            if (ret < 0) {
                                av_log(NULL, AV_LOG_ERROR, "Skip-Option: Error setting filter param %d\n", ret);
                                exit_program(1);
                            }
                        }
                        else {
                            av_log(NULL, AV_LOG_ERROR, "Skip-Option: No trim filter available\n");
                            exit_program(1);
                        }
                    }
                    else {
                        av_log(NULL, AV_LOG_ERROR, "Skip-Option: No inpute filters present\n");
                        exit_program(1);
                    }

                    seek_timestamp = is->skip_position;

                    if (!(is->iformat->flags & AVFMT_SEEK_TO_PTS)) {
                        int dts_heuristic = 0;
                        for (i=0; i< is->nb_streams; i++) {
                            const AVCodecParameters *par = is->streams[i]->codecpar;
                            if (par->video_delay)
                                dts_heuristic = 1;
                        }
                        if (dts_heuristic) {
                    ...
                    ret = avformat_seek_file(is, -1, INT64_MIN, seek_timestamp, seek_timestamp, 0);

                    av_log(NULL, AV_LOG_INFO, "Skip-Option - Seek performed to: %s input_cur_dts: %s\n",
                        av_ts2timestr(is->skip_position, &AV_TIME_BASE_Q),
                        av_ts2timestr(ist->dts, &AV_TIME_BASE_Q));
...
static void subtitle_kickoff(InputStream *ist, int64_t pts)
{
    ...
    if (ist->st->codecpar->codec_type != AVMEDIA_TYPE_VIDEO || pts == AV_NOPTS_VALUE)
        return;
...
static int init_input_stream(int ist_index, char *error, int error_len)
{
    ...
    if (ist->decoding_needed) {
        ...
        // For subtitles, we can't wait for a frame to arrive (might never happen)
        // so we need to set the format early
        if (ist->dec_ctx->codec_type == AVMEDIA_TYPE_SUBTITLE)
            for (int i = 0; i < ist->nb_filters; i++)
                if (ist->filters[i]->format == -1)
                    ist->filters[i]->format = ist->dec_ctx->subtitle_type;
    ...
    if (ost->encoding_needed) {
        ...
        if (ist && ist->dec->type == AVMEDIA_TYPE_SUBTITLE && ost->enc->type == AVMEDIA_TYPE_SUBTITLE) {
            ...
            if (ist->nb_filters == 0 && in_subtitle_format != AV_SUBTITLE_FMT_UNKNOWN && out_subtitle_format != AV_SUBTITLE_FMT_UNKNOWN
                ...
                if (out_subtitle_format == AV_SUBTITLE_FMT_BITMAP)
                    snprintf(error, error_len, "To convert text subtitle to graphical subtitles you need to use the text2graphicsub filter");
                else
                    snprintf(error, error_len, "To convert graphical subtitle to text subtitles you need to use the graphicsub2text filter");
                return AVERROR_INVALIDDATA;
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (!locate_option(argc, argv, options, "version"))
        print_local_time();
